...
classpath = "/workspace/src/com/iotcube/util/sort-gen-0.3.0-lib.jar" # 
jpype.startJVM(
    jpype.getDefaultJVMPath(),
    "-Djava.class.path={classpath}".format(classpath=classpath),
    convertStrings=True,
)
jpkg = jpype.JPackage('com.iotcube.tools.sortgen') ## java 위치
VersionSortUtil = jpkg.VersionSortUtil()

def compareVersion(compareVersion, targetVersion):
    return VersionSortUtil.compareVersion(compareVersion, targetVersion)

def sortVersions(versions):
    return VersionSortUtil.sortVersions(versions)
